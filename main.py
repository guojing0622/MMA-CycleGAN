...
def sample_images(batches_done, epoch_sample=False):
    """Saves a generated sample from the test set"""
    if epoch_sample == True:
        ...
        save_image(img_sample, 'images/%s/%s/%s.png' % (opt.dataset_name, 'test result', batches_done),
...
image_horse = image_horse.permute(0, 3, 1, 2)
...
image_zebra = image_zebra.permute(0, 3, 1, 2)

for epoch in range(opt.epoch, opt.n_epochs):
    # Loss weights
    if epoch < 70 :
        lambda_GAN = 5 - (epoch-20)//10
        lambda_cyc = 5 + (epoch-20)//10
    ...
    for i, batch in enumerate(dataloader):
        ...
        #fake_B, fake_A = G(real_A_, real_B_)
        #recov_A,_ = G(torch.cat((fake_B, label_B),1), real_A_)
        #recov_B,_ = G(torch.cat((fake_A, label_A),1), real_B_)
        fake_B, fake_A = G(real_A, real_B)
        recov_A, _ = G(fake_B, real_A)
        recov_B,_ = G(fake_A, real_B)


        # Identity loss
        # loss_id_B = criterion_identity(G(real_B_, real_B_)[0], real_B)
        loss_id_B = criterion_identity(G(real_B, real_B)[0], real_B)
        # GAN loss
        ...
        # Identity loss
        # loss_id_A = criterion_identity(G(real_A_, real_A_)[0], real_A)
        loss_id_A = criterion_identity(G(real_A, real_A)[0], real_A)
        # GAN loss
